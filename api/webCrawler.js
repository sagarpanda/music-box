...
function crawler(argUrl, { querySelector, forScreen }) {
  const urlPath = argUrl || 'https://mymp3singer.fun/categorylist/2899/latest_bollywood_mp3_song/default/1';
...
function movies(req, res) {
  ...
  const url = `https://mymp3singer.fun/categorylist/2899/latest_bollywood_mp3_song/default/${page}`;
  const querySelector = dom => dom.window.document.querySelector('.catList').querySelectorAll('a');
  ...
  Model.find({ url }, { _id: 0, __v: 0 }, (err, records) => {
    if (records.length === 0) {
      ...
      console.log('movies api - no crawling');
